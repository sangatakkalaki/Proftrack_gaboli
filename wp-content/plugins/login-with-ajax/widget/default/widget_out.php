...
?>
	<div class="lwa lwa-default"><?php //class must be here, and if this is a template, class name should be that of template directory ?>
		...
		<form class="lwa-form" action="<?php echo esc_attr(LoginWithAjax::$url_login); ?>" method="post">
						...
						<a href="<?php echo esc_attr(LoginWithAjax::$url_register); ?>" class="lwa-links-register lwa-links-modal" onclick ='$.fancybox.close();'><?php esc_html_e('Sign Up','login-with-ajax') ?></a></p>
                        <?php endif; ?>
						<span class = 'log'>
						<?php do_action( 'wordpress_social_login'); ?></span>
        	</div>
			<div id= 'after'>
            ...
            <table>
                <tr class="lwa-username">
                    ...
                    <td class="lwa-username-input">
                        <input type="text" name="log" placeholder="USERNAME & EMAIL"/>
                    </td>
                </tr>
        ...
        <?php endif; ?>
		<?php if( get_option('users_can_register') && !empty($lwa_data['registration']) ): ?>
		<div class="lwa-register lwa-register-default lwa-modal" style="display:none;" id = 'reg'>
		...
		<!--	<h4><?php //esc_html_e('Register For This Site','login-with-ajax') ?></h4>
			...
			<p id = "sign_up">OR SIGN UP WITH EMAIL:</p>
			<div id = 'reg_form'>
			<form class="lwa-register-form" action="<?php echo esc_attr(LoginWithAjax::$url_register); ?>" method="post">
				...
				<p class="lwa-username">
					...
					<input type="text" name="user_login" id="user_login" class="input" size="20" tabindex="10" placeholder = 'USERNAME'/></label>
				</p>
				<p class="lwa-email">
					<label><?php //esc_html_e('E-mail','login-with-ajax') ?><br />
					<input type="text" name="user_email" id="user_email" class="input" size="25" tabindex="20" placeholder = 'E-MAIL'/></label>
				</p>
				<?php do_action('register_form'); ?>
				<?php do_action('lwa_register_form'); ?>
				<p class="submit">
					<input type="submit" name="wp-submit" id="wp-submit" class="button-primary" value="<?php //esc_attr_e('Register', 'login-with-ajax'); ?> Sign Up" tabindex="100" />
				</p>
			...
			<div id = 'terms'>
					<span class = 'terms_o'>We are committed to protecting your privacy.</span>
					<p class = 'terms_c'>By proceeding you agree to abide by our <a href = 'http://www.proftrack.com/beta/privacy-statements/ '><span class = 'pstc'>Privacy Statement</span></a> and our <a href = 'http://www.proftrack.com/beta/terms-conditions/' ><span class = 'pstc' >Terms and conditions </span></a>.</p>
				</div>
		</div>
		<?php endif; ?>
	</div>